...
CLIENT_PROJECT="$ROOT/src/GamerTodo.Client/GamerTodo.Client.csproj"
OUTPUT_ROOT="$ROOT/artifacts/releases"
PUBLISH_DIR="$OUTPUT_ROOT/client-$RUNTIME"
APP_DIR="$OUTPUT_ROOT/GamerTodo.app"
APP_CONTENTS="$APP_DIR/Contents"
APP_MACOS="$APP_CONTENTS/MacOS"
APP_RESOURCES="$APP_CONTENTS/Resources"
DMG_STAGING="$OUTPUT_ROOT/dmg-root"
DMG_PATH="$OUTPUT_ROOT/GamerTodo-$RUNTIME-$VERSION.dmg"
...
<dict>
    ...
    <string>GamerTodo</string>
    <key>CFBundleExecutable</key>
    <string>GamerTodo.Client</string>
    <key>CFBundleIdentifier</key>
    <string>com.gamertodo.client</string>
...
chmod +x "$APP_MACOS/GamerTodo.Client" || true

rm -rf "$DMG_STAGING"/*
cp -R "$APP_DIR" "$DMG_STAGING/GamerTodo.app"

if command -v hdiutil >/dev/null 2>&1; then
    rm -f "$DMG_PATH"
    hdiutil create -volname "GamerTodo" -srcfolder "$DMG_STAGING" -ov -format UDZO "$DMG_PATH"
