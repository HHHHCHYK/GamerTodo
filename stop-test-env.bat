...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Get-CimInstance Win32_Process | Where-Object { $_.Name -in @('dotnet.exe', 'GamerTodo.Client.exe') -and ($_.CommandLine -match 'GamerTodo\.Client' -or $_.CommandLine -match [regex]::Escape('%ROOT%src\GamerTodo.Client')) } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"
if errorlevel 1 (
